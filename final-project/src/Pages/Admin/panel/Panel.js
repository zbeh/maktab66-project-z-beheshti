...
export default function Panel() {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:3002/orders", { params: { token: token } })
  ...
  return (
    <>
      <div className={panelStyles.navigation}>
        <Box className={`${panelStyles.box} container`}>
          <Link className={panelStyles.link} to="products">
            کالاها
          </Link>
          <Link className={panelStyles.link} to="quantity">
            {" "}
            موجودی و قیمت ها
          </Link>
          <Link className={panelStyles.link} to="orders">
            سفارش ها
          </Link>
        </Box>
      </div>
