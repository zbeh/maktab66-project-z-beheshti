  ...
  return (
    <>
      <div className={productDetailStyles.proContainer} >
        {loading ? (
          <div className={`${productDetailStyles.center} d-flex justify-center`}>
             <CircularProgress />
            </div>
         ...
         <div className={`${productDetailStyles.container} d-flex`}>
           <img src={`http://localhost:3002/files/${data.data[0].images[0]}`} />
           <div className={productDetailStyles.info}>
             <p>{data.data[0].name}</p>
             <p>{data.data[0].price}تومان</p>
             <div className="d-flex">
                <input type="number" name="count"  min="1" max={`${data.data[0].count}`}/>
             <Button className={productDetailStyles.btn}>
               افزودن به سبد خرید
             </Button>
             </div>

             <p dangerouslySetInnerHTML={{__html: data.data[0].description}} />
           </div>
         </div>
       </div> 
